...
export async function POST(req: NextRequest) {
  try {
    ...
    // Reporter-authored public note: written to potentially be shown
    // publicly, but stays hidden (public_note_approved defaults to false
    // in the DB) until an admin specifically approves it -- see
    // supabase/schema.sql. Optional, capped at 500 chars server-side too
    // in case the client-side maxLength gets bypassed.
    const reporterPublicNote = ((formData.get('reporter_public_note') as string) || '').trim().slice(0, 500) || null;
    ...
    const { data: inserted, error: insertError } = await supabase
      ...
      .insert({
        ...
        reporter_public_note: reporterPublicNote,
